...
public class MPPPeriod extends X_PP_Period {
	 ...
	 * @return MPeriod
	 */
	public static MPPPeriod findByCalendar(Properties ctx, Timestamp startdate, Timestamp enddate,
			int M_Calendar_ID, String trxName) {
		...
		while (it.hasNext()) {
			MPPPeriod period = (MPPPeriod) it.next();
			if (period.getM_Calendar_ID() == M_Calendar_ID
					&& period.isInPeriod(startdate)
					&& period.getAD_Client_ID() == AD_Client_ID)
				return period;
		}

		String whereClause = "PP_PeriodDefinition_ID IN (SELECT PP_PeriodDefinition_ID FROM PP_PeriodDefinition WHERE PP_Calendar_ID = ?) " // maximea
				+ " AND ? = TRUNC(StartDate, 'DD') AND ? = TRUNC(EndDate, 'DD')";

		MPPPeriod period = new Query(ctx, I_PP_Period.Table_Name, whereClause,
				trxName).setClient_ID()
				.setParameters(M_Calendar_ID, TimeUtil.getDay(startdate), TimeUtil.getDay(enddate))
				.setOnlyActiveRecords(true).first();
