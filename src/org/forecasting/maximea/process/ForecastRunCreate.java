...
public class ForecastRunCreate extends SvrProcess {
	...
	private int generateForecastRunMaster(MPPForecastRun run,
		...
		for (MPPForecastDefinitionLine fdl : fd.getLines(true)) {
			final StringBuffer select = new StringBuffer(
					// added , sh.C_BPartner_ID
					"SELECT DISTINCT sh.M_Product_ID , sh.M_Warehouse_ID, sh.C_BPartner_ID FROM C_SalesHistory sh LEFT JOIN  PP_ForecastRunMaster m ON (m.M_Product_ID=sh.M_Product_ID AND ");
			...
			try {
				...
				while (rs.next()) {
					...
					master.setC_BPartner_ID(C_BPartner_ID);//maximea
					master.saveEx();
				}
			} catch (Exception e) {
	...
	public int generateForcastRunLines(MPPForecastRunDetail frd) {
		...
		insertSQL.append(" AND ").append(MSalesHistory.COLUMNNAME_C_BPartner_ID) //maximea
				.append(" =? ");
		...
		parameters.add(frd.getPP_ForecastRunMaster().getC_BPartner_ID());//maximea	
